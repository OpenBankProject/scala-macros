...
class CacheKeyFromArgumentsMacro(val c: blackbox.Context) {
  ...
  private def commonMacroImpl[F[_], V: c.WeakTypeTag](keyNameToCachingCall: (c.TermName) => c.Tree): Tree = {
    ...
    val className = showRaw(classSymbol.fullName)
